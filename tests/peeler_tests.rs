...
fn test_unescape_string_normal_cases() {
    ...
    let result = peeler.unescape_string("line1\\nline2\\tindented\\\"quoted\\\"").unwrap();
...
"#;
    ...
    if let Value::Mapping(map) = &yaml_value {
        if let Some(Value::Mapping(data)) = map.get(&Value::String("data".to_string())) {
            if let Some(Value::String(json_content)) = data.get(&Value::String("config.json".to_string())) {
            ...
            if let Some(Value::String(normal_content)) = data.get(&Value::String("normal-file".to_string())) {
